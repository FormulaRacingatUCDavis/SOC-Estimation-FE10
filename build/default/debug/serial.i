...
SerialResult_t Serial_Update(Serial_t* serial)
{
    while(byte_available())
    {
        ...
        if(serial->escaped)
        {
            ...
            {
                serial->rx_buf[serial->rx_index] = 0x05;
                serial->rx_index++;
            }
            else
            {
        ...
        else
        {
            ...
            {
                serial->rx_buf[serial->rx_index] = b;
                serial->rx_index++;
            }
        }
    }
...
SerialResult_t Serial_GetMsg(Serial_t* serial, SerialMsg_t* msg)
{
    if(serial->rx_index < 2)
    {
        return SERIAL_NOMSG;
    }

    msg->dlc = serial->rx_buf[0];
    uint8_t expected_len = msg->dlc + 2;
    if(serial->rx_index < expected_len)
    {
        return SERIAL_NOMSG;
    }

    msg->id = serial->rx_buf[1];
...
void Serial_SendMsg(SerialMsg_t* msg)
{
    ...
    send_byte_with_escape(msg->id);
    send_byte_with_escape(msg->dlc);
    for(uint8_t i = 0; i < msg->dlc; i++)
    {
        send_byte_with_escape(msg->buf[i]);
    }
}
